...
namespace WindowsHookEx
{
    ...
    {
        internal KeyPressEventArgsExt(char keyChar, int timestamp, bool isInjectedKey)
            : base(keyChar)
        {
            IsNonChar = keyChar == (char) 0x0;
            Timestamp = timestamp;
            IsInjectedKey = isInjectedKey;
        }
        ...
        public KeyPressEventArgsExt(char keyChar)
            : this(keyChar, Environment.TickCount, false)
        {
        }
        ...
        /// <summary>
        ///     True if event was injected into the system.
        /// </summary>
        public bool IsInjectedKey { get; }
        ...
        internal static IEnumerable<KeyPressEventArgsExt> FromRawDataApp(CallbackData data)
        {
            ...
            const uint maskInjectedKey = 0x00000010;
            ...
            var isInjectedKey = (flags & maskInjectedKey) > 0;
        ...
        internal static IEnumerable<KeyPressEventArgsExt> FromRawDataGlobal(CallbackData data)
        {
            ...
            const uint maskInjectedKey = 0x00000010;
            var isInjectedKey = (fuState & maskInjectedKey) > 0;

            if (virtualKeyCode == KeyboardNativeMethods.VK_PACKET)
            {
                var ch = (char) scanCode;
                yield return new KeyPressEventArgsExt(ch, keyboardHookStruct.Time, isInjectedKey);
            }
            else
            {
                char[] chars;
                ...
                foreach (var current in chars)
                    yield return new KeyPressEventArgsExt(current, keyboardHookStruct.Time, isInjectedKey);
            }
        }
    }
}
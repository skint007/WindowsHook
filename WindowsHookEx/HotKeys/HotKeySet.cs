...
namespace WindowsHookEx.HotKeys
{
    /// <summary>
    ...
    {
        /// <summary>
        ///     A delegate representing the signature for the OnHotKeyDownHold event
        /// </summary>
        ...
        /// </summary>
        public IEnumerable<Keys> HotKey { get; }
        ...
        public bool Enabled
        {
            ...
            {
                if (value)
                    InitializeKey(); //must get the actual current state of each key to update
        ...
        ///     <see cref="OnHotKeyDownOnce" />
        /// </summary>
        public event HotKeyHandler OnHotKeyDownHold;

        /// <summary>
        ///     Called whenever the hot key set is no longer active.  This is essentially a KeyPress event, indicating that a full
        ///     key cycle has occurred, only for HotKey because a single key removed from the set constitutes an incomplete set.
        /// </summary>
        public event HotKeyHandler OnHotKeyUp;
        ...
        /// </summary>
        public event HotKeyHandler OnHotKeyDownOnce;
        ...
        /// <returns>
        ///     True if successful.  False doesn't indicate a failure to unregister, it indicates that the Keys is not
        ///     registered as an Exclusive Or key or it's not the Primary Keys.
        /// </returns>
        ...
        private void OnKeyDown(Keys k)
        {
            ...
            if (HotKeyActivated)
            {
                InvokeHotKeyHandler(OnHotKeyDownHold); //Call the duration event
            }
            ...
            else if (m_hotkeystate.ContainsKey(k) && !m_hotkeystate[k])
            {
                ...
                if (HotKeyActivated) //because of the increase, check whether the set is activated
                    InvokeHotKeyHandler(OnHotKeyDownOnce); //Call the initial event
            }
        }
        ...
        private void OnKeyUp(Keys k)
        {
            ...
            {
                var wasActive = HotKeyActivated;

                m_hotkeystate[k] = false; //key's state is up
                --m_hotkeydowncount; //this set is no longer ready

                if (wasActive)
                    InvokeHotKeyHandler(OnHotKeyUp); //call the KeyUp event because the set is no longer active
            }
        }
    }
}